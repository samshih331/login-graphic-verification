...
exports.serve = (options) => {
    ...
    app.post('/render', (req, res) => {
        ...
        saveRedis(options.redisPort, options.redisHost, options.redisPassword, key, answer, ttl);

        let dataURL = renderPic(answer, width, height).toDataURL();
        res.set('X-Captcha-Token', key)
        res.send(dataURL)
    });
}
...
saveRedis = (port = '6379', host = '127.0.0.1', password, key, code, expireTime = 60) => {
